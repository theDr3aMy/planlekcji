...
/**********************************************************WYKONAŁ: KAMIL PTAK 1dP****************************************************************/

const toggle = document.getElementById('toggleDark');
const tdlast = document.getElementById('l-last');
const tdl = document.getElementsByClassName('l');
const tdlbk = document.getElementsByClassName('l-bk');
...
toggle.addEventListener('click', function () {
	...
	if (this.classList.toggle('bi-brightness-high-fill')) {
		...
		tdlast.style.background = 'white';
		for (let x = 0; tdl.length > x; x++) {
			tdl[x].style.backgroundColor = "white";
		}
		for (let x = 0; tdlbk.length > x; x++) {
			tdlbk[x].style.backgroundColor = "white";
		}
		body.style.transition = '2s';
	}
});
